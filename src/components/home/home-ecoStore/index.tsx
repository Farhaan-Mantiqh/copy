...
import ecoStoreProducts from '@/overallData/json/home-ecostore-pro.json';
...
import type {
  ...
  EcoStoreProduct,
  HeadingConfig,
} from '@/types/ecoStore-types';

export default function HomeEcoStore(): JSX.Element {
  const filteredProducts = useMemo(() => {
    return ecoStoreProducts
      .filter((product: EcoStoreProduct) => !product?.out_of_stock)
      .slice(0, 7)
      .map((product) => ({
  ...
  const homeEcoStoreHeading: HeadingConfig = useMemo(() => {
    return {
      beforeText: 'Redeem your',
      highlightedText: 'ecocoins',
      afterText: 'by choosing from our selection of ecofriendly products',
    };
  }, []);

  const renderItem = useCallback(
    (item: EcoStoreItem, openModal: () => void) => {
      console.log('this are the images', item.imgSrc);
      return (
        <div className="relative aspect-[81/100] md:aspect-[7/10]">
          ...
          <div className="absolute inset-0 flex h-full w-full flex-col justify-end rounded-md bg-gradient-to-t from-black/70 via-black/30 to-transparent p-3 md:p-4 lg:p-6">
            ...
            <div className="flex items-center justify-between pt-2 md:pt-3 lg:pt-4">
              ...
              <Button
                onClick={openModal}
                className="ml-1 cursor-pointer rounded-[30px] border-none bg-ecocrewPrimary px-3 py-[6px] leading-4 transition-all ease-in-out hover:scale-95 md:px-3 md:py-[6px] md:leading-[18px] lg:px-5 lg:py-[10px]">
  ...
  return (
    <div className="mx-auto max-w-[1440px]">
      <EcoCard
        sectionTitle="ECO STORE"
        heading={homeEcoStoreHeading}
        exploreButtonText="Explore ecostore"
        exploreButtonLink="/eco-store"
